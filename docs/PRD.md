...
| Field | Value |
| --- | --- |
| Status | Draft v1.6 — dynamic analytics, reporting and logical object/data wireframe contract |
...
| Implementation state | React/TypeScript/Vite synthetic prototype now includes a high-fidelity Salesforce Lightning-style internal shell, both product surfaces, all 12 contracted screen families, a canonical cross-route scenario state, functional operational workflows, 11 permission-filtered dynamic dashboard views with reconciled cards/charts/detail rows and 18 displayed metric contracts, and an interactive logical object/data studio covering 92 object families, 48 `DAT-*` groups and 920 minimum logical data-point definitions; Salesforce metadata, physical fields/API names, production analytics, BFF/auth, provider integration and approved pilot environment do not exist |
...
| v1.6 change boundary | Public-safe dynamic analytics and logical object/data wireframe release: application source, deterministic fixtures, tests, visual evidence, README, Pages deployment evidence and this PRD may change; no Salesforce metadata/org, physical field model, production backend, authentication, provider write, real candidate data or pilot authorization is created |
...
| What exists now? | A local synthetic v0.9 React prototype and bounded automated evidence; the v1.2 PRD contract | Accountable prototype acceptance, verified remote deployment and every production layer remain unproven |
...
This section is the canonical inventory answer for product review. It distinguishes a contracted screen family from a final page/component, a human persona from a permission or machine identity, a logical data group from an atomic field, and a logical object family from a physical Salesforce object. Counts from different grains must not be added together or presented as implementation sizing.
...
| Contracted screen/route families | 12 | 4 candidate `UI-CAN-*` plus 8 internal `UI-HR-*` | This is the only approved exact screen count; one family may later split into several pages, tabs, modals or responsive compositions |
...
| `WP-01` Prototype engineering foundation | React/TypeScript/Vite, routing/base path, design tokens, fixture generator, accessibility/test harness, CI and Pages | Prototype-only decisions in `WP-00`; no production providers | M | **Locally implemented, acceptance incomplete:** clean-checkout build/test evidence exists; remote CI/Pages, rollback and accountable acceptance remain unverified |
...
At v0.9, local synthetic prototype evidence exists for parts of `EVD-001`, `EVD-002` and `EVD-007`; remote CI/Pages, accountable usability/accessibility/content acceptance and every real-candidate launch gate remain unproven. `ART-010` records individual test cases and reproducible evidence; this table groups ownership and closure expectations.
...
| Synthetic prototype | `M4 — Implemented` locally; bounded `M5 — Verified` evidence | The v0.9 application, fixtures and automated checks exist locally; remote CI/Pages, rollback, moderated usability, manual accessibility and accountable content acceptance remain unverified |
...
The next legitimate maturity movement is accountable v1.2 workshop/decision closure, employer baseline/learning-plan preparation and completion of `WP-01/02` prototype acceptance, followed by the separately approved `WP-03`–`WP-07` production foundation and control work. This v1.2 documentation release does not itself authorize implementation, use of real candidate data or a pilot.
