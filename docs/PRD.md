...
| Field | Value |
| --- | --- |
| Status | Draft v3.2 — connected use-case and action-level DFD synthetic wireframe contract |
...
| Implementation state | The local React/TypeScript/Vite v3.2 synthetic prototype exposes 72 screen contracts and 166 declared routes/164 functional destinations. It retains the v3.1 Demo Journey Studio and adds 12 connected outcome-driven use cases, 52 action-level DFD processes, 13 seeded P0 workbenches/78 workbench fields, a shared version/event/handoff/metric ledger and all 35 `WF-P0/P1/P2` updates. All 138 declared object families retain 552 role-aware List-New-Detail-Edit pages; every action remains browser-memory-only and no production service or physical schema exists |
...
| Full-audit state | [v3.2 connected wireframe completion audit](AUDIT-v3.2.md) reconciles all 35 `WF-P0/P1/P2` items, screens, routes, interactions and automated evidence; the defined v3.2 wireframe backlog is complete, while all production and accountable human-evidence findings remain Open until dated evidence exists |
| v3.2 change boundary | Synthetic interaction release: implement all 35 prioritized wireframe updates across connected use-case workbenches, trust controls and demo operations; do not claim deployable Salesforce metadata, production backend, authentication, persistent data, provider writes, real candidate/employee data, legal approval or pilot authorization |
...
### 1.13 v3.1 demo-journey and business data-flow brief

v3.1 turns the complete v3.0 surface into a repeatable demo operating model. It does not add a database, API, production identity or external effect. [MATRIX-v3.1.md](MATRIX-v3.1.md) is the authoritative demo-pack, journey, data-flow, persona-handoff and evidence ledger.

| Demo-readiness question | v3.1 wireframe answer | Boundary retained |
| --- | --- | --- |
| Can the platform be demonstrated as business outcomes rather than a screen tour? | Eight audience packs combine 12 business use cases into executive, recruiter, candidate, onboarding, high-volume, ecosystem, partner and governance narratives | A facilitator must still rehearse the selected pack against the audience and available time |
| Are all previously catalogued journeys represented? | Yes. `JRN-001` through `JRN-084` map every journey unit to an actor, business-use-case diagram, outcome and live wireframe route | A mapped route is not evidence of production completion or moderated usability |
| Are journeys represented as data-flow diagrams? | Yes. `DFD-01` through `DFD-12` show external entities, named input/output flows, business processes, governed stores and outcome evidence; every process opens its live wireframe step | Diagrams describe logical business flow, not deployed service topology, database schema or provider architecture |
| Can a presenter hand the same story across personas? | A persistent presenter rail retains the active use case, record context, variant, step, expected receipt and next actor; internal handoffs update the existing persona projection | Portal selection and persona switching remain browser simulations, not authentication or server authorization |
| Are demonstrations deterministic and recoverable? | Launch and restart load known scenario fixtures; happy and exception modes are selectable; completed steps append a browser-memory receipt and selected steps update shared scorecard, offer, availability, onboarding, provisioning or HRIS projections | Refresh clears browser memory and no durable replay/event service exists |
| Is action evidence visible? | A session evidence ledger records demo receipt, synthetic time, actor, step, action, outcome, variant and logical store; communication previews are explicitly marked unsent | Evidence is presentation proof only and cannot satisfy production audit, signature, message-delivery or provider requirements |
| Can dashboards be narrated from upstream actions? | The guided flows link transactional actions to analytics, readiness, work queues and recovery routes, while existing shared state recalculates selected readiness projections | Full causal metric recomputation across all 84 journeys remains a production semantic-layer and integration obligation |

The executable route contract becomes 66 screen contracts and 160 route declarations/158 functional destinations with two redirects. The new demo layer adds four screen contracts—command center, journey catalogue, business data-flow detail and session evidence—plus 61 guided handoff steps. The underlying product remains 13 actor personas, 12 internal roles, 138 object families, 552 generated CRUD pages, 175 logical concepts and zero approved physical objects.

#### v3.1 demo acceptance contract

Every business-use-case record must declare:

- audience, duration, objective and business outcome;
- actors, logical objects, data stores and communication previews;
- deterministic scenario fixture and start route;
- ordered actor/process/input/output/store handoffs;
- expected receipt and exception/recovery result;
- mapped journey units and live wireframe routes.

Every guided demo step must support direct opening, previous-step navigation, receipt recording, next-actor handoff and full deterministic restart. No feature is considered missing solely because a real database, API, identity provider, message, calendar, signature or provider effect is absent from this synthetic GitHub Pages wireframe.

### 1.14 v3.2 use-case, screen, action, DFD and feature brief

v3.2 implements the next wireframe iteration. [USE-CASE-MATRIX-v3.2.md](USE-CASE-MATRIX-v3.2.md) is the authoritative screen-by-screen contract for 12 outcome-driven recruitment, selection, offer, handoff, onboarding and worker-transition use cases, with machine-readable evidence in `artifacts/v3.2/*`.

| Definition question | v3.2 answer | Current boundary |
| --- | --- | --- |
| What is the unit of demonstration? | A business use case with a trigger, accountable actors, prerequisites, exact screens/actions, named inputs/outputs/events/stores, controls, exceptions, recovery and measurable outcome | Implemented through the v3.2 connected workbench; v3.1 guided journeys remain available |
| How many use cases are executable? | 12, from hiring demand to published job through day-one/day-90 activity and worker transitions | They refine the existing 12 level-one DFDs; they do not add production integrations |
| What did v3.2 add? | All 35 updates: 15 P0 end-to-end requirements, 12 P1 trust/operational-depth requirements and 8 P2 facilitation improvements | Completion is synthetic-wireframe evidence only |
| How is cross-route state handled? | Each material action updates the feature version, event/receipt, downstream handoff, audit timeline and governed KPI; selected inherited product projections also update | State lasts only for the browser session and resets deterministically |
| How did DFDs deepen? | 52 processes expose route, actor action, input, output, object/version, event, guard, denial and recovery | They are logical business/action contracts, not deployed service topology |
| How is wireframe completion judged? | A reviewer can launch deterministic data, perform material actions, follow cross-persona handoffs, inspect data/event changes, exercise failure/cancellation/recovery, verify audit/dashboard consequences and reset safely | Real identity, persistence, delivery, signatures, providers, HRIS and approved legal content remain production-only |

The implemented P0 sequence is structural: connected cross-route state and level-two DFDs; requisition/job publication; audience/identity/source controls; partner and high-volume flows; interview/debrief; screening; offer; PreHire/PendingWorker; onboarding readiness; and worker-transition compensation. P1 adds universal lineage, state diffs, handoff inboxes, rule explanations, complete audit, dashboard reconciliation, persona data-scope inspection, country variants, comprehensive error states, governed reporting and human accessibility/usability evidence hooks.

The executable v3.2 baseline is 13 actor personas, 12 internal roles, 72 screen contracts, 166 route declarations/164 functional destinations, 138 object families, 552 generated CRUD pages, 12 broad v3.1 DFDs, 84 mapped journey units, 61 guided handoffs, eight demo packs, 12 connected use cases, 52 action-level DFD processes and all 35 v3.2 update contracts. None of this synthetic evidence closes a production, manual human-evidence or pilot gate.

The [v3.2 completion audit](AUDIT-v3.2.md) records 313 buttons, 319 links and 166 route declarations with zero static defects; 130 passing unit/component/contract/automated-accessibility tests; 73 passing desktop/mobile browser tests with one intentional duplicate-crawl skip; and an exhaustive 1,103-destination production-build crawl with zero route defects or browser errors. Desktop and mobile visual checks have zero horizontal overflow. Manual accountable accessibility/usability evidence and public Pages deployment remain separate gates.

The revised repeatable narrated demo artifact covers Business Case 01, hiring demand to published job. Its 21-chapter, 7:18 sequence opens with the business outcome and progressively animates all five action-level DFD processes before demonstrating the real seeded routes: manager-owned `REQ-DEMO-001` draft v3; exact-version submission; independent Finance and Compensation approvals to approved v4; recruiter configuration of `JOB-DEMO-001` and two opening records; posting-v7 readiness and four-destination publication; the candidate-safe public projection; isolated LinkedIn failure and targeted recovery; causal analytics; and acknowledged cross-persona handoff. The 1,920×1,080 client master includes Indian-English narration, cursor and field focus choreography, 71 embedded and sidecar sentence-level English caption cues, 21 embedded chapters, transcript, thumbnail, production metadata and a 16-frame visual-QA sheet under `artifacts/v3.2/videos/`. A 3:16 captioned executive cut and stream-copied detailed evidence master are packaged from the same verified source. This is presentation evidence for the synthetic browser-memory wireframe, not moderated usability, production-operation, provider-delivery or pilot evidence.

The complete v3.2 demo-video library now covers all 12 business use cases in numbered, self-contained folders under `artifacts/v3.2/business-case-video-library/`. Every set contains a narrated 1,920×1,080 client demo, shorter executive cut, detailed evidence master, embedded and sidecar English captions, full and executive transcripts, narration contract, metadata, thumbnails and a 12-frame case-level visual-QA sheet. The library contains 36 categorized MP4 entries, 49.2 minutes of client demonstrations, 32.4 minutes of executive cuts, 120 embedded client chapters and 474 sentence-level client caption cues. Each UC-02–UC-12 video opens with the business outcome, dynamically highlights every action-level DFD process, visits the actual product route, explains the six governed workbench fields, executes the primary transition, demonstrates the controlled exception and targeted recovery, and closes through the causal dashboard and next-actor handoff. The executable preflight also corrected the stale `CASE-DEMO-001` adverse-action fixture while retaining the documented 32 screening-case count. `QA-REPORT.md` and `qa-report.json` record successful required-file, 1080p video, 48 kHz audio, embedded-caption, chapter/cue, duration and full video/audio decode checks across all 12 sets. These remain presentation artifacts for deterministic browser-memory behavior, not proof of production integrations, legal approval, real delivery, moderated usability or pilot operation.
...
| --- | --- | --- |
| 3.2 | August 29, 2026 | Implemented the complete 35-item connected-wireframe release: 12 outcome-driven use cases, 52 action-level DFD processes, 13 seeded P0 workbenches/78 fields, shared cross-route versions/events/receipts/handoffs/KPIs, universal lineage/diff/rules/audit/communication/access/country/error/report/accessibility controls, 10/30/60-minute runbooks, named scenarios, checkpoints/bookmarks, happy/exception comparison, print briefs, talk tracks, rehearsal classification and feedback; expanded to 72 screen contracts and 166 routes/164 destinations; retained browser-memory-only behavior and every production/manual/pilot gate |
| 3.1 | August 28, 2026 | Added the deterministic Demo Journey Studio to the full v3.0 surface: four screen contracts, 12 level-one business DFDs, 84 mapped journey units, 61 guided persona/process handoffs, eight audience demo packs, happy/exception launch and restart, shared-state projections, communication previews, presenter rail and session evidence; reconciled the executable contract to 66 screens and 160 routes/158 destinations while retaining browser-memory-only behavior and every production gate |
