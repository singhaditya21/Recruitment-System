...
import { referralRewards } from "../data/lifecycleDepth";
...
function ReferralWorkspace({ announce }: { announce: (message: string) => void }) {
  ...
  const rewards = referralRewards.filter((reward) => rewardState === "All" || reward.state === rewardState);
  const fullIdentity = ["Recruiter", "Recruiting Coordinator", "Privacy & Legal"].includes(persona.role);
  return <div className="referral-workspace"><section className="panel lifecycle-table-panel"><div className="panel-heading"><div><h2>Employee referral ledger</h2><span>Referral relationship, candidate permission, conflicts and rewards remain separate.</span></div><button disabled={!fullIdentity} className="primary-button" onClick={() => announce("Referral invitation form opened in preview; the candidate still controls whether an application is created.")}><UserPlus size={15} /> Invite referral</button></div><div className="table-toolbar"><label><Filter size={15} /><select aria-label="Filter referral state" value={state} onChange={(event) => setState(event.target.value)}><option>All</option><option>Invited</option><option>Submitted</option><option>Review</option><option>Converted</option><option>Not selected</option><option>Withdrawn</option></select></label><Pill tone="info">{rows.length} referrals</Pill></div><div className="referral-table" role="region" aria-label="Employee referrals"><div aria-hidden="true"><span>Referral</span><span>Person and role</span><span>Relationship</span><span>State</span><span>Conflict</span><span>Reward</span></div>{rows.map((referral) => <div key={referral.id}><span><strong>{referral.id}</strong><small>{referral.submitted}</small></span><span><strong>{fullIdentity ? referral.prospect : `Restricted prospect · ${referral.id}`}</strong><small>{referral.job} · referred by {referral.referrer}</small></span><span>{referral.relationship}</span><span><Pill tone={referral.state === "Converted" ? "success" : referral.state === "Review" ? "warning" : "info"}>{referral.state}</Pill></span><span><Pill tone={referral.conflictCheck === "Review required" ? "warning" : "success"}>{referral.conflictCheck}</Pill></span><span>{referral.rewardState}</span></div>)}</div></section><section className="panel reward-ledger"><div className="panel-heading"><div><span className="eyebrow">Policy-governed reward cases</span><h2>Referral eligibility, milestones and disputes</h2><span>Commercial credit cannot change candidate review or reveal restricted candidate outcomes.</span></div></div><div className="table-toolbar"><label><Filter size={15} /><select aria-label="Filter referral reward state" value={rewardState} onChange={(event) => setRewardState(event.target.value)}><option>All</option><option>Pending</option><option>Approved</option><option>Paid</option><option>Denied</option><option>Disputed</option></select></label><Pill tone="info">{rewards.length} reward cases</Pill></div><div className="reward-grid">{rewards.map((reward) => <article key={reward.id}><div><Pill tone={reward.state === "Paid" ? "success" : reward.state === "Disputed" ? "danger" : reward.state === "Approved" ? "info" : "warning"}>{reward.state}</Pill><small>{reward.id} · {reward.referralId}</small></div><h3>{fullIdentity ? reward.candidate : `Restricted referral · ${reward.referralId}`}</h3><p>{reward.job} · referred by {reward.referrer}</p><dl><div><dt>Eligibility</dt><dd>{reward.eligibility}</dd></div><div><dt>Milestone</dt><dd>{reward.milestone}</dd></div><div><dt>Amount</dt><dd>{reward.amount}</dd></div><div><dt>Reason</dt><dd>{reward.reason}</dd></div></dl><button disabled={!fullIdentity} className="secondary-button" onClick={() => announce(`${reward.id} reward review opened; candidate decision data remains excluded.`)}>Review reward</button></article>)}</div></section></div>;
}
