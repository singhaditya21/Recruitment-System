...
export function Freshness({ children = "Fixture snapshot · 3 min ago" }: { children?: React.ReactNode }) {
...
export function ExplainPanel({ title = "Why this is here", children, source }: { title?: string; children: React.ReactNode; source?: string }) {
  return (
    <div className="explain-panel" role="note">
      <div className="explain-icon"><Database size={16} aria-hidden="true" /></div>
      <div>
        <strong>{title}</strong>
        <p>{children}</p>
...
export function IntegrityNotice({ kind = "human" }: { kind?: "human" | "simulation" | "restricted" }) {
  const copy = {
    human: { icon: ShieldCheck, title: "Human decision required", text: "The system can explain readiness and blockers. It cannot rank, select or reject a person." },
    simulation: { icon: Beaker, title: "Simulation only", text: "This action previews expected records and side effects. It cannot call a live service." },
    restricted: { icon: AlertTriangle, title: "Restricted workspace", text: "Prototype roles demonstrate least-privilege boundaries; no authentication or real access is implemented." },
  }[kind];
  const Icon = copy.icon;
  return (
    <div className={`integrity-notice integrity-${kind}`}>
      <Icon size={18} aria-hidden="true" />
      <div><strong>{copy.title}</strong><span>{copy.text}</span></div>
    </div>
  );
}
  ...
  return (
    <ol className="stepper" aria-label="Application progress">
      {steps.map((step, index) => (
        <li className={index < current ? "complete" : index === current ? "current" : ""} key={step} aria-current={index === current ? "step" : undefined}>
...
export function Metric({ value, label, detail, tone = "neutral" }: { value: string; label: string; detail?: string; tone?: Tone }) {
  return (
