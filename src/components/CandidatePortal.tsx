  ...
  return (
    <CandidateShell>
      <div className="candidate-page hub-page">
        ...
        <div className="candidate-hub-tabs" role="tablist" aria-label="Candidate hub sections">{["Applications", "Profile & privacy", "Messages"].map((item) => <button role="tab" aria-selected={hubTab === item} onClick={() => setHubTab(item)} key={item}>{item === "Applications" ? <BriefcaseBusiness size={17} /> : item === "Profile & privacy" ? <LockKeyhole size={17} /> : <Mail size={17} />}{item}</button>)}</div>
